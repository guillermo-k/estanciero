    ...
    while opcion !=0 or habilitado:
        ...
        for propiedad in jugador.propiedades:
            if isinstance(propiedad,tablero.Campo) and propiedad.completo and propiedad.edificaciones < 5:
                opciones_activas.append(2)
                break
        if jugador.chacras>0:
            opciones_activas.append(3)
        opciones_activas.append(4)
        if len(jugador.propiedades) > jugador.hipotecadas:
            opciones_activas.append(5)
        if jugador.hipotecadas>0:
            opciones_activas.append(6)
        opciones_activas.append(7)
        if len(jugadoresActivos)>2:
            opciones_activas.append(8)
        if not habilitado:
            opciones_activas.append(0)
...
def transferir(comprador,vendedor,monto,campo):
    ...
    if campo.estado=='Hipotecado':
        ...
        while True:
            respuesta=vistas.mostrar_mensaje(((comprador.nombre,1), (campo.nombre+' se encuentra bajo hipoteca.',0), ('Desea levantar la hipoteca en este momento',0), ('por $'+str(campo.valorHipotecado)+'?',0), ('Recuerde que si no lo hace ahora',0),('cuando lo haga debera pagar un adicional del 10%',0)),True)
    ...
    for player in jugadoresActivos:
        while player.dinero<0:
            ...
            if opcion == 'edifcaciones':
                ...
                vistas.mostrar_mensaje(((player.nombre,1),('Ha sido declarado en BANCARROTA',0),('y debe abandonar el juego.',0),('Mas suerte en la proxima',1)))
...
def main():
    while len(jugadoresActivos) > 1:
        for jugador in jugadoresActivos:
            turno(jugador)
            comprobarBancarrota()
    vistas.mostrar_mensaje(((jugadoresActivos[0].nombre,1),('FELICITACIONES',1),('USTED HA GANADO',1),('EL JUEGO',1)))
