...
class Jugador:
    ...
    def __str__(self) -> str:
        return f"""El jugador {self.nombre} tiene asignado el peon {self.peon}
        cuenta con ${self.dinero} y se encuentra en la posicion
        {self.ubicacion} su orden es: {self.orden}"""
    ...
    def tirarDados(self, pantalla=None, x=None, y=None):
        ...
        pack_dados = (pack_dado1, pack_dado2, pack_dado3,
                      pack_dado4, pack_dado5, pack_dado6)
        if pantalla:
            pygame.draw.rect(pantalla, (0, 0, 0), (x/2-200, y/2-150, 400, 300),
                             border_radius=25)
