...
def mostrar_mensaje(mensaje,botones=False):
    ...
    if botones:
        pygame.draw.rect(pantalla,constantes.rojo,(x/3,y/4+(y/10)*len(mensaje),x/3,y/10),border_radius=int(y/20))
        btn_si=pygame.draw.rect(pantalla,constantes.verde,(x/2-x/9,y/4+(y/10)*len(mensaje)+y/40,x/10,y/20),border_radius=int(x/25))
        si = fuente_botones.render('SI',True,constantes.negro)
        rect_si = si.get_rect()
        rect_si.center = btn_si.center
        btn_no=pygame.draw.rect(pantalla,constantes.verde,(x/2+x/9-x/10,y/4+(y/10)*len(mensaje)+y/40,x/10,y/20),border_radius=int(x/25))
        no = fuente_botones.render('NO',True,constantes.negro)
        rect_no = no.get_rect()
        rect_no.center = btn_no.center
        pantalla.blits(blit_sequence=((si,rect_si),(no,rect_no)))
    else:
        btn_cerrar = pygame.draw.rect(pantalla,constantes.rojo,(x/2-x/20,y/4+(y/10)*len(mensaje)-y/40,x/10,y/20),border_radius=int(x/25))
...
def comerciar(comerciables,jugadoresActivos,jugador):
    ...
    for propiedad in comerciables:
        if propiedad.titular==jugador.nombre:
            ...
            otros_propiedads.append(propiedad)
    prop_ofrecidas = propiedades_comerciar(mis_propiedades,jugadoresActivos,'Ofrecer')
    prop_pedidas = propiedades_comerciar(otros_propiedads,jugadoresActivos,'Pedir')
...
def rematar(jugador,jugadores_activos):
    if len(jugadores_activos)>2:
        ...
        while len(otros)>1 or (len(otros)==1 and oferta_vigente[0]==0):
            ...
            for oferente in otros:
                ...
                while seguir:
                    ...
                    for otro in otros:
                        btn_jugador = (pygame.draw.rect(pantalla,constantes.verde,(x/300,y/10 + y/11 * (otros.index(otro)+1),x/6,y/20)),otro)
                        nombre = fuente_botones.render(otro.nombre,True,constantes.negro)
                        rect_nombre = nombre.get_rect()
                        rect_nombre.center = btn_jugador[0].center
                        pantalla.blit(nombre,rect_nombre)
                    cont +=1
                    ventana = pygame.draw.rect(pantalla,constantes.blanco,(x/4,y/4,x/2,y/2),0,int(x/15))
                    btn_mas_cien = pygame.draw.rect(pantalla,constantes.azul,(x/2-x/20,3*y/4.5,x/10,y/20),0,int(y/50))
                    ...
                    btn_ofertar = pygame.draw.rect(pantalla,constantes.verde,(x/2+x/8-x/20,3*y/4.5,x/10,y/20),0,int(y/50))
                    ...
                    btn_declinar = pygame.draw.rect(pantalla,constantes.rojo,(x*(13/40),3*y/4.5,x/10,y/20),0,int(y/50))
                    ...
                    cuadro_texto = pygame.draw.rect(pantalla,constantes.negro,(2*x/5,y/2+y/20,x/5,y/15),0,int(x/15))
                    importe = fuente_encabezado.render((texto_importe),True,constantes.verde)
                    rect_importe = importe.get_rect()
                    rect_importe.centery = cuadro_texto.centery
                    rect_importe.right = cuadro_texto.right-x/50
                    ...
                    en_subasta = fuente_botones.render('Subasta: '+a_subastar.nombre+' ('+a_subastar.estado+')',True,constantes.negro)
                    rect_en_subasta = en_subasta.get_rect()
                    rect_en_subasta.center = (x/2,y/3+y/15)
                    actual = fuente_botones.render('La oferta actual es: $'+str(oferta_vigente[0])+' por '+str(oferta_vigente[1].nombre),True,constantes.negro)
                    rect_actual = actual.get_rect()
                    rect_actual.center = (x/2,y/2)
                    pantalla.blits(blit_sequence=((nombre,rect_nombre),(en_subasta,rect_en_subasta),(actual,rect_actual),(text_declinar,rect_text_declinar),(text_ofertar,rect_text_ofertar),(text_mas_cien,rect_text_mas_cien),(signo,rect_signo),(cursor,rect_cursor),(importe,rect_importe)))
                    for evento in pygame.event.get():
                        if evento.type == pygame.QUIT:
                            pygame.quit()
                        elif evento.type == pygame.MOUSEBUTTONDOWN:
                            ...
                            if btn_ofertar.collidepoint(raton):
                                ...
                                if int(texto_importe)>oferta_vigente[0]:
                                    ...
                                    mostrar_mensaje((('La oferta debe ser superior a la vigente',0),('',0)))
                        elif evento.type == pygame.KEYDOWN:
                            if evento.key == pygame.K_BACKSPACE:
                                texto_importe = texto_importe[:-1]
                            elif evento.unicode.isdigit():
        ...
        if oferta_vigente[0] > 0:
            mostrar_mensaje(((oferta_vigente[1].nombre,1),('Ha resultado ganador de la subasta',1),('Ha comprado '+a_subastar.nombre,0),('por $'+str(oferta_vigente[0]),0)))
            return oferta_vigente,a_subastar


    else:
        mostrar_mensaje(((jugador.nombre,1),('Para realizar una subasta',0),('debe haber por lo menos 2 jugadores aparte de usted',0)))


def saldo_negativo(jugador,jugadores_activos):
    pintar_tablero(pantalla,largo_utilizable,x,y,jugadores_activos)
    info_jugador(jugador,jugadores_activos)
    mostrar_mensaje(((jugador.nombre,1),('Su saldo es: -$'+str(-jugador.dinero),0),('debera saldar su deuda antes de poder seguir',0),('de no poder hacerlo, sera declarado en',0),('BANCARROTA',1),('y quedara fuera de juego.',0)))
    pintar_tablero(pantalla,largo_utilizable,x,y,jugadores_activos)
    seguir = True
    while seguir:
        ...
        color1,color2,color3,color4=constantes.verde,constantes.verde,constantes.verde_oscuro,constantes.verde_oscuro
        ...
        btn_edificaciones = (pygame.draw.rect(pantalla,color1,(x_de_botones,y/4 + 3*y/20,ancho_botones,y/30),0,int(y/15)),'edifcaciones',btn_edificaciones_habilitado)
        btn_hipotecar = (pygame.draw.rect(pantalla,color2,(x_de_botones,y/4 + 4*y/20,ancho_botones,y/30),0,int(y/15)),'hipotecar',btn_hipotecar_habilitado)
        btn_propiedades = (pygame.draw.rect(pantalla,color3,(x_de_botones,y/4 + 5*y/20,ancho_botones,y/30),0,int(y/15)),'propiedades',btn_propiedades_habilitado)
        btn_rematar = (pygame.draw.rect(pantalla,color4,(x_de_botones,y/4 + 6*y/20,ancho_botones,y/30),0,int(y/15)),'rematar',btn_rematar_habilitado)
        btn_bancarrota = (pygame.draw.rect(pantalla,constantes.rojo,(x_de_botones,y/4 + 8*y/20,ancho_botones,y/30),0,int(y/15)),'bancarrota',btn_bancarrota_habilitado)
        pygame.draw.rect(pantalla,constantes.negro,(x_de_botones,y/4 + 8*y/20,ancho_botones,y/30),2,int(y/15))
        ...
        pantalla.blits(blit_sequence=((linea1,rect_linea1),(linea2,rect_linea2),(linea3,rect_linea3),(linea4,rect_linea4),(linea5,rect_linea5),(linea6,rect_linea6),(txt_edificaciones,rect_txt_edificaciones),(txt_hipotecar,rect_txt_hipotecar),(txt_propiedades,rect_txt_propiedades),(txt_rematar,rect_txt_rematar),(txt_bancarrota,rect_txt_bancarrota)))
        pygame.display.update()
        for evento in pygame.event.get():
            if evento.type == pygame.QUIT:
                pygame.quit()
            ...
            if evento.type == pygame.QUIT:
                ...
                for btn_peon in btns_peones:
                    if btn_peon[0].collidepoint(raton):
                        txt_nombre, peon_seleccionado,seguir = comprobar_nombre(num_jugador,jugadoresActivos,btn_peon[1],txt_nombre)
            elif evento.type == pygame.KEYDOWN:
                if evento.key == pygame.K_RETURN or evento.key == pygame.K_KP_ENTER:
                    peon = constantes.peones[0]
                    txt_nombre, peon_seleccionado,seguir = comprobar_nombre(num_jugador,jugadoresActivos,peon,txt_nombre)
                elif evento.key == pygame.K_BACKSPACE:
                    txt_nombre = txt_nombre[:-1]
                else:
                    txt_nombre += evento.unicode
                    txt_nombre = txt_nombre.capitalize()
        pantalla.blits(blit_sequence=((renglon1,rect_renglon1),(nombre,rect_nombre),(cursor,rect_cursor),(renglon2,rect_renglon2)))
        pygame.display.update()
...
def fuentes(ancho):
    ...
    return (fuente_numero,fuente_provincia,fuente_nombre,fuente_valor,fuente_gigante, fuente_grande)

def dibujar_rectangulo_inclinado(pantalla,casillero,vertice1:tuple,ancho:int,alto:int,jugadores,seleccionables,accion,seleccionados=[],angulo=False):
    ...
    vertice2=(math.cos(angulo_rad)*ancho+vertice1[0],vertice1[1]-math.sin(angulo_rad)*ancho)
    vertice4=(math.cos(angulo_rad+math.pi/2)*alto+vertice1[0],vertice1[1]-math.sin(angulo_rad+math.pi/2)*alto)
    vertice3=(math.cos(angulo_rad)*ancho+vertice4[0],vertice4[1]-math.sin(angulo_rad)*ancho)
    vertices=(vertice1,vertice2,vertice3,vertice4)
    centro = (vertice1[0] - math.sin(math.radians(angulo + 14)) * alto * 0.9 ,vertice1[1] - math.cos(math.radians(angulo + 14)) * alto * 0.9)
    ...
    for j_en_casillero in en_casillero:
        medio_arriba = ((vertice1[0] + vertice2[0])/2,(vertice1[1] + vertice2[1])/2)
        centro_peon = (medio_arriba[0] - math.sin(math.radians(angulo)) * alto/(len(en_casillero)+1) * (en_casillero.index(j_en_casillero)+1) ,medio_arriba[1] - math.cos(math.radians(angulo)) * alto/(len(en_casillero)+1) * (en_casillero.index(j_en_casillero)+1))
...
def pintar_esquina(pantalla,casillero,vertice1:tuple,ancho:int,alto:int,jugadores,espesor:int=0,ang=False):
    ...
    angulo_rad=math.radians(angulo)
    vertice2 = (vertice1[0] + math.cos(angulo_rad+math.radians(270))*alto, vertice1[1] - math.sin(angulo_rad+math.radians(270))*alto)
    vertice3 = (vertice1[0] + math.cos(angulo_rad+math.radians(300))*alto*1.155, vertice1[1] - math.sin(angulo_rad+math.radians(300))*alto*1.155)
    vertice4 = (vertice1[0] + math.cos(angulo_rad+math.radians(330))*alto, vertice1[1] - math.sin(angulo_rad+math.radians(330))*alto)
    vertices=(vertice1,vertice2,vertice3,vertice4)
    pygame.draw.polygon(pantalla,casillero.color,vertices)
    pygame.draw.polygon(pantalla, constantes.negro,vertices,2)
    centro = (vertice1[0] + math.sin(math.radians(angulo + 30)) * alto * 0.95 ,vertice1[1] + math.cos(math.radians(angulo + 30)) * alto * 0.95)
    en_casillero = []
    for jugador in jugadores:
        if jugador.ubicacion == casillero.numero:
            en_casillero.append(jugador)
    peones=[]
    for j_en_casillero in en_casillero:
        centro_peon = (vertice1[0] + math.sin(math.radians(angulo + 30)) * alto/(len(en_casillero)+1) * (en_casillero.index(j_en_casillero)+1), vertice1[1] + math.cos(math.radians(angulo + 30)) * alto/(len(en_casillero)+1) * (en_casillero.index(j_en_casillero)+1))
        peones.append((centro_peon,j_en_casillero.peon))
...
def rellenar_casillero(pantalla,centro,ancho,casillero,angulo,alto,peones):
    ...
    if isinstance(casillero,tablero.Campo):
        ...
        linea1 = (centro[0] + math.cos(math.radians(angulo-90))*alto*0.6, centro[1] - math.sin(math.radians(angulo-90))*alto*0.6)

        nombre = fuente_nombre.render(casillero.provincia ,True, constantes.negro)
        linea2 = (centro[0] + math.cos(math.radians(angulo-90))*alto*0.5, centro[1] - math.sin(math.radians(angulo-90))*alto*0.5)

        zona = fuente_provincia.render('ZONA '+casillero.zona,False, constantes.negro)
        linea3=(centro[0] + math.cos(math.radians(angulo-90))*alto*0.3, centro[1] - math.sin(math.radians(angulo-90))*alto*0.3)

        valor = fuente_valor.render('Valor $'+str(casillero.valor),True, constantes.negro)
        linea4=(centro[0] + math.cos(math.radians(angulo-90))*alto*0.2, centro[1] - math.sin(math.radians(angulo-90))*alto*0.2)
        ...
        linea1 = (centro[0] + math.cos(math.radians(angulo-90))*alto*0.7, centro[1] - math.sin(math.radians(angulo-90))*alto*0.7)
        linea2 = (centro[0] + math.cos(math.radians(angulo-90))*alto*0.55, centro[1] - math.sin(math.radians(angulo-90))*alto*0.55)
        linea3 = (centro[0] + math.cos(math.radians(angulo-90))*alto*0.4, centro[1] - math.sin(math.radians(angulo-90))*alto*0.4)
        linea4 = (centro[0] + math.cos(math.radians(angulo-90))*alto*0.2, centro[1] - math.sin(math.radians(angulo-90))*alto*0.2)
        ...
        linea1 = (centro[0] + math.cos(math.radians(angulo-90))*alto*0.6, centro[1] - math.sin(math.radians(angulo-90))*alto*0.6)
        linea2 = (centro[0] + math.cos(math.radians(angulo-90))*alto*0.25, centro[1] - math.sin(math.radians(angulo-90))*alto*0.25)
        ...
        valor = fuente_valor.render('Valor $'+str(casillero.valor),True, constantes.negro)
        linea_imagen = (centro[0] + math.cos(math.radians(angulo-90))*alto*0.75, centro[1] - math.sin(math.radians(angulo-90))*alto*0.75)
        linea1 = (centro[0] + math.cos(math.radians(angulo-90))*alto*0.58, centro[1] - math.sin(math.radians(angulo-90))*alto*0.58)
        linea2 = (centro[0] + math.cos(math.radians(angulo-90))*alto*0.5, centro[1] - math.sin(math.radians(angulo-90))*alto*0.5)
        linea3 = (centro[0] + math.cos(math.radians(angulo-90))*alto*0.4, centro[1] - math.sin(math.radians(angulo-90))*alto*0.4)
        linea_valor = (centro[0] + math.cos(math.radians(angulo-90))*alto*0.2, centro[1] - math.sin(math.radians(angulo-90))*alto*0.2)
        ...
        if casillero.numero == 18 or casillero.numero == 22:
            ...
            linea4 = (centro[0] + math.cos(math.radians(angulo-90))*alto*0.32, centro[1] - math.sin(math.radians(angulo-90))*alto*0.32)
        ...
        linea = (centro[0] + math.cos(math.radians(angulo-90))*alto*0.45, centro[1] - math.sin(math.radians(angulo-90))*alto*0.45)
        valor = fuente_valor.render('Valor $'+str(casillero.valor),True, constantes.negro)
        linea_valor = (centro[0] + math.cos(math.radians(angulo-90))*alto*0.2, centro[1] - math.sin(math.radians(angulo-90))*alto*0.2)
        imagen = pygame.transform.scale(pygame.image.load(png),size)
        linea_imagen = (centro[0] + math.cos(math.radians(angulo-90))*alto*0.7, centro[1] - math.sin(math.radians(angulo-90))*alto*0.7)
        ...
        if casillero.numero == 0:
            ...
            linea1 = (centro[0] + math.cos(math.radians(angulo-90))*alto*0.8, centro[1] - math.sin(math.radians(angulo-90))*alto*0.8)
            linea2 = (centro[0] + math.cos(math.radians(angulo-90))*alto*0.65, centro[1] - math.sin(math.radians(angulo-90))*alto*0.65)
            linea3 = (centro[0] + math.cos(math.radians(angulo-90))*alto*0.5, centro[1] - math.sin(math.radians(angulo-90))*alto*0.5)
            linea4 = (centro[0] + math.cos(math.radians(angulo-90))*alto*0.35, centro[1] - math.sin(math.radians(angulo-90))*alto*0.35)
            linea5 = (centro[0] + math.cos(math.radians(angulo-90))*alto*0.2, centro[1] - math.sin(math.radians(angulo-90))*alto*0.2)
            linea6 = (centro[0] + math.cos(math.radians(angulo-90))*alto*-0.0, centro[1] - math.sin(math.radians(angulo-90))*alto*-0.0)
            ...
            linea = (centro[0] + math.cos(math.radians(angulo-90))*alto*0.25, centro[1] - math.sin(math.radians(angulo-90))*alto*0.25)
            ...
            linea1 = (centro[0] + math.cos(math.radians(angulo-90))*alto*0.35, centro[1] - math.sin(math.radians(angulo-90))*alto*0.35)
            linea2 = (centro[0] + math.cos(math.radians(angulo-90))*alto*0.25, centro[1] - math.sin(math.radians(angulo-90))*alto*0.25)
            textos = (numero, renglon1, renglon2)
            centros = (centro,linea1, linea2)
            if casillero.numero == 7:
                ...
                linea3 = (centro[0] + math.cos(math.radians(angulo-90))*alto*0.6, centro[1] - math.sin(math.radians(angulo-90))*alto*0.6)
                linea4 = (centro[0] + math.cos(math.radians(angulo-90))*alto*0.5, centro[1] - math.sin(math.radians(angulo-90))*alto*0.5)
...
def pintar_tablero(pantalla,largo_utilizable,x,y,jugadoresActivos,seleccionables=[],accion='',seleccionados=[]):
    ...
    for casillero in tablero.tablero:
        if casillero.numero in (0,7,14,21,28,35):
            ...
            dibujar_rectangulo_inclinado(pantalla,casillero,vertice1,ancho_casillero,-alto_casillero,jugadoresActivos,seleccionables,accion,seleccionados)
    ...
    if isinstance(casillero,tablero.Casillero):
        while escape:
            ...
            if casillero.numero in (0,7,14,21,28,35):
                ...
                centros = ((x_inicial+860,140),(x_inicial+860,180),(x_inicial+860,260),(x_inicial+860,300),(x_inicial+480,140),(x_inicial+480,530))
                textos = []
                ...
                if isinstance(casillero, tablero.Campo):
                    ...
                    chacra2 = fuente_alquileres.render('Idem con 2 chacra        '+str(casillero.alquiler2Chacras),1, constantes.negro)
                    chacra3 = fuente_alquileres.render('Idem con 3 chacra        '+str(casillero.alquiler3Chacras),1, constantes.negro)
                    chacra4 = fuente_alquileres.render('Idem con 4 chacra        '+str(casillero.alquiler4Chacras),1, constantes.negro)
                    ...
                    centros = ((x_inicial+480,180),(x_inicial+480,220),(x_inicial+480,250),(x_inicial+480,280),(x_inicial+480,310),(x_inicial+480,340),(x_inicial+480,370),(x_inicial+480,400),(x_inicial+480,430),(x_inicial+480,460),(x_inicial+480,490))
                    escribir_textos(pantalla,textos,centros,0)
                elif isinstance(casillero, tablero.Compañia):
                    ...
                    centros = ((x_inicial+480,180),(x_inicial+480,213),(x_inicial+480,254),(x_inicial+480,290),(x_inicial+480,312),(x_inicial+480,344),(x_inicial+480,366),(x_inicial+480,398),(x_inicial+480,420))
                    textos = [nombre]
                    for texto in textos1:
                    ...
                    centros = ((x_inicial+480,180),(x_inicial+480,213),(x_inicial+480,254),(x_inicial+480,290),(x_inicial+480,320),(x_inicial+480,350),(x_inicial+480,380))
                    textos = [nombre]
                    for texto in textos1:
                        textos.append(fuente_alquileres.render(texto,1, constantes.negro))
                    escribir_textos(pantalla,textos,centros,0)
            else:
                dibujar_rectangulo_inclinado(pantalla,casillero,(x/2-115,50),230,-560,[],[],'',angulo=360)
            pygame.display.flip()
            for evento in pygame.event.get():
    ...
    for casillero in tablero.tablero:
        ...
        if casillero.numero in (0,7,14,21,28,35):
            ...
            if casillero.numero == 21 or casillero.numero == 28:
                ...
                and raton[0] > v1[0] - dis_a_y1 / math.tan(math.radians(casillero.angulo-30.00000001))):
                    return casillero
            if casillero.numero == 14:# or casillero.numero == 35:
